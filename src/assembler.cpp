...
int Assembler::first_pass(const char *line, int lineptr)
{
  ...
  if (instruction(ptr) >= 0) {
    ...
    if (CODEORDATA != CODE) {
      sprintf(txt,"Pasada 1 - Error en linea %d\nInstruccion en memoria de datos",lineptr);
      std::cout << txt << "\n";
      return 1;
    }
 ...
 case REG2C:
  ...
  if (!error) {
	...
	error = TRUE;
      break;
    case J_TYPE:
      if (in_range(w,0x3ffffff)) 
	code_word = (op | (w&0x3ffffff));
      else 
	error=TRUE;
      break;
    case F_TYPE:
...
BOOL Assembler::directive(int pass, const char *ptr, const char *line) {
  ...
  if (ptr == NULL || *ptr != '.')
	...
	if (pass == 1) {
	  dataptr += num;
	  if (!cpu->data->isValidAddress(dataptr))
	    return FALSE;
	}
	if (pass == 2) {
	  cpu->data->setAddressDescription(dataptr, line);
