...
void CWinEVEDoc::check_stalls(int status,const char *str,int rawreg,char *txt)
{
	char mess[100];
	if (status==RAW)	
	{
...
int CWinEVEDoc::update_io(processor *cpu)
{
	...
	case (WORD32)4:
		...
		if (fp.u<cpu->datasize) cpu->Terminal+= std::string((const char *)az);
...
int CWinEVEDoc::mygets(char *line,int max,FILE *fp)
{
	if (feof(fp)) {
		return 1;
	}
...
BOOL CWinEVEDoc::getcodesym(const char *&ptr,WORD32 *m)
{
    return getsym(code_table,code_symptr,ptr,m);
}

BOOL CWinEVEDoc::getdatasym(const char *&ptr,WORD32 *m)
{
    return getsym(data_table,data_symptr,ptr,m);
}

int CWinEVEDoc::instruction(const char *start)
{
    int i=0;
...
BOOL CWinEVEDoc::directive(int pass,const char *ptr,const char *line)
...
int CWinEVEDoc::first_pass(const char *line,int lineptr)
{
    int i,len;
    const char *ptr=line;
	char txt[100];
...
int CWinEVEDoc::second_pass(const char *line,int /* lineptr */)
{
    WORD32 w,byte;
...
void CWinEVEDoc::dump_mem() {
	BYTE *mem = cpu.code;
	std::cout << "Code Size: " << cpu.codesize << std::endl;
	std::cout << "Data Size: " << cpu.datasize << std::endl;
...
void CWinEVEDoc::dump_Terminal() {
	if (cpu.Terminal == "") 
		return;
	std::cout << "Terminal: ";
	std::cout << cpu.Terminal << std::endl;
	cpu.Terminal = "";
}

void CWinEVEDoc::show_stats() {
...
void CWinEVEDoc::show_screen() {
	std::cout << "----- Pantalla -----" << std::endl;
	WORD32 *pantalla = cpu.screen;	
